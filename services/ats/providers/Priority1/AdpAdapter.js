 ...
 * using Puppeteer-Extra with Stealth Plugin (cloud-ready) with Playwright fallback.
 */
class AdpAdapter extends BaseAdapter {
  ...
  async searchJobs() {
    ...
    // Engine 1: Puppeteer Extra with Stealth (Standard in Linux containers)
    try {
      rawJobs = await this._scrapeViaPuppeteer(targetUrl);
    } catch (puppeteerErr) {
      console.warn(`[AdpAdapter] Puppeteer scrape failed (${puppeteerErr.message}), trying Playwright fallback...`);
      try {
        rawJobs = await this._scrapeViaPlaywright(targetUrl);
      } catch (pwErr) {
        console.error(`[AdpAdapter] Both Puppeteer and Playwright failed:`, pwErr.message);
        throw new Error(`ADP Scraping failed: ${puppeteerErr.message} | Fallback: ${pwErr.message}`);
      }
    }
  ...
  async _scrapeViaPuppeteer(targetUrl) {
  ...
  async _scrapeViaPlaywright(targetUrl) {
    ...
    try {
      ...
      await page.goto(targetUrl, { waitUntil: 'domcontentloaded', timeout: 45000 });
      await page.waitForSelector('.card, h2.card-title', { timeout: 15000 }).catch(() => {});
