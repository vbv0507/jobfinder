...
const getSkipReason = (job, profile) => {
    ...
    const technicalRoleMatches =
        /\b(software|sde|backend|front.?end|full.?stack|developer|engineer|node\.?js|express|mongodb|javascript|rest api|api developer)\b/i.test(text);
    const fresherRoleMatches =
        /intern|internship|fresher|graduate|new grad|entry level|campus|trainee|0\s*-\s*1|0\s*to\s*1/i.test(text);
    ...
    const hasSeniorKeyword =
        /\b(senior|sr\.|lead|principal|manager|director|architect|staff)\b/i.test(text);
    const hasTwoPlusYears = [...text.matchAll(/(\d+)\s*\+?\s*(?:-|to)?\s*(\d+)?\s*(?:years?|yrs?)/g)]
