...
const getScraperHealth = async (req, res) => {
    try {
        const Company = require('../models/Company');
        const companies = await Company.find({}, 'name lastSuccess lastFailure failureReason atsProvider successPercent avgResponseTime jobsFound lastScrapedAt active').sort({ name: 1 });
        res.json(companies);
    } catch (e) {
        res.status(500).json({ error: e.message });
    }
